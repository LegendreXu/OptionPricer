...
class Option:
    def __init__(self, type, S0, K, t, rf, sigma, dv = 0):
        ...
        self.rf = rf * 1.0
        self.dv = dv * 1.0
        self.d_1 = (np.log(self.S0 / self.K) + (self.rf - self.dv + .5 * self.sigma ** 2) * self.t) / self.sigma / np.sqrt(self.t)
...
Label(root, text='Designed by XU Guosong').grid(columnspan=3)
cp = StringVar()
